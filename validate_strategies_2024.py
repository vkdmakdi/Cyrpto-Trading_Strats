...
FEATURE_FILE = Path("data/backtest_results/strategy_validation_2024.csv")
...
def load_validation_features() -> pd.DataFrame:
    ...
    # Hard guard: this file must truly be 2024 validation data.
    if not f["signal_time"].between(VALIDATION_START, VALIDATION_END).all():
        raise ValueError(
            "Validation feature file contains rows outside 2024. "
...
def main():
    ...
    print("=" * 72)
    print("LOCKED 2024 VALIDATION")
    print("=" * 72)
    print("Period: 2024-01-01 through 2024-12-31 UTC")
    print("Horizon: 24h")
    ...
    print(f"2024 original all-RSI signals: {len(signals)}")
    ...
    if signals["RSI30_delta_4"].isna().any():
        ...
        raise ValueError(f"{n} 2024 signals are missing RSI30_delta_4.")
    ...
    summary_path = OUTPUT_DIR / "strategy_validation_2024_summary.csv"
    trades_path = OUTPUT_DIR / "strategy_validation_2024_trades.csv"
    diagnostics_path = OUTPUT_DIR / "strategy_validation_2024_diagnostics.csv"
